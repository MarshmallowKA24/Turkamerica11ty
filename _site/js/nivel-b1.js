// =====================================================
// NIVEL B1 - SEARCH AND MODAL FUNCTIONALITY
// =====================================================

// Cache global para las explicaciones
let b1ExplanationsCache = null;

// Función para obtener las explicaciones del archivo JSON
async function getB1Explanations() {
    if (b1ExplanationsCache) {
        return b1ExplanationsCache;
    }

    try {
        const response = await fetch('/data/b1_lessons.json');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        b1ExplanationsCache = data;
        return data;
    } catch (error) {
        console.error("Error al cargar las lecciones de B1:", error);
        document.getElementById('modalTitle').textContent = "Error de Carga";
        document.getElementById('modalContent').innerHTML = "<p>No se pudo cargar el contenido de las lecciones. Asegúrate de que Eleventy se ejecutó correctamente y el archivo <code>b1_lessons.json</code> existe en la carpeta <code>_site/data/</code>.</p>";
...
async function openExplanation(topic) {
    ...
    const explanations = await getB1Explanations();
