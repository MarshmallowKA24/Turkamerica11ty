// =====================================================
// NIVEL A2 - SEARCH AND MODAL FUNCTIONALITY
// =====================================================

// Cache global para las explicaciones
let a2ExplanationsCache = null;

// Función para obtener las explicaciones del archivo JSON
async function getA2Explanations() {
    if (a2ExplanationsCache) {
        return a2ExplanationsCache;
    }

    try {
        const response = await fetch('/data/a2_lessons.json');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        a2ExplanationsCache = data;
        return data;
    } catch (error) {
        console.error("Error al cargar las lecciones de A2:", error);
        document.getElementById('modalTitle').textContent = "Error de Carga";
        document.getElementById('modalContent').innerHTML = "<p>No se pudo cargar el contenido de las lecciones. Asegúrate de que Eleventy se ejecutó correctamente y el archivo <code>a2_lessons.json</code> existe en la carpeta <code>_site/data/</code>.</p>";
...
async function openExplanation(topic) {
    ...
    const explanations = await getA2Explanations();
