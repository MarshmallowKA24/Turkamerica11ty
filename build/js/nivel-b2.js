// =====================================================
// NIVEL B2 - SEARCH AND MODAL FUNCTIONALITY
// =====================================================

// Cache global para las explicaciones
let b2ExplanationsCache = null;

// Función para obtener las explicaciones del archivo JSON
async function getB2Explanations() {
    if (b2ExplanationsCache) {
        return b2ExplanationsCache;
    }

    try {
        const response = await fetch('/data/b2_lessons.json');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        b2ExplanationsCache = data;
        return data;
    } catch (error) {
        console.error("Error al cargar las lecciones de B2:", error);
        document.getElementById('modalTitle').textContent = "Error de Carga";
        document.getElementById('modalContent').innerHTML = "<p>No se pudo cargar el contenido de las lecciones. Asegúrate de que Eleventy se ejecutó correctamente y el archivo <code>b2_lessons.json</code> existe en la carpeta <code>_site/data/</code>.</p>";
...
async function openExplanation(topic) {
    ...
    const explanations = await getB2Explanations();
