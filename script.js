...
function setupMessageListener() {
    window.addEventListener('message', function(event) {
        ...
        switch(event.data.type) {
            ...
            case 'deleteCard':
                const { deckId: deleteDeckId, cardId: deleteCardId } = event.data;
                console.log(`Eliminando tarjeta ${deleteCardId} del deck ${deleteDeckId}`);
                deleteCardFromDeck(deleteDeckId, deleteCardId);
                break;
...
// NUEVA FUNCIÓN: Eliminar tarjeta específica de cualquier deck
function deleteCardFromDeck(deckId, cardId) {
    console.log('=== INICIO deleteCardFromDeck ===');
    console.log('Parámetros recibidos:', { deckId, cardId });
    ...
    // Eliminar la tarjeta del deck
    deck.cards.splice(cardIndex, 1);
    console.log(`🗑️ Tarjeta "${card.front}" eliminada del deck "${deck.name}"`);
    
    // Si es un deck de aprendidas y queda vacío, eliminarlo
    if (deck.isLearnedDeck && deck.cards.length === 0) {
        decks = decks.filter(d => d.id !== deck.id);
        console.log(`🗑️ Deck de aprendidas "${deck.name}" eliminado por estar vacío`);
    }
    ...
    console.log('=== FIN deleteCardFromDeck ===');
}
function deleteCardFromDeck(deckId, cardId) {
    console.log('=== INICIO deleteCardFromDeck ===');
    console.log('Parámetros recibidos:', { deckId, cardId });
